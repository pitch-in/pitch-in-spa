defmodule PitchIn.Accounts.User do
  use Ecto.Schema

  import Ecto.Changeset
  import PitchIn.Shared.Validators

  alias PitchIn.Accounts.User

  alias PitchIn.Shared.Tokens

  alias PitchIn.Accounts.Pro
  ...
  def volunteer_registration_changeset(%User{} = struct, params \\ %{}) do
    struct
    |> registration_changeset(params)
    |> cast_assoc(:pro)
  end

  def staff_registration_changeset(%User{} = struct, params \\ %{}) do
    struct
    |> registration_changeset(params)
  ...
  def password_changeset(%User{} = struct, params \\ %{}) do
    struct
    |> changeset(params)
  ...
  def forgot_password_changeset(%User{} = struct, params \\ %{}) do
    token = Tokens.token
  ...
  def complete_changeset(%User{} = struct) do
    struct
    |> change(is_complete: true)
  end

  def staffer_changeset(%User{} = struct) do
    struct
    |> change(is_staffer: true)
  end

  defp registration_changeset(%User{} = struct, params \\ %{}) do
    struct
    |> changeset(params)
